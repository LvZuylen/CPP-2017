...
void Mobiel::verzendNr(const std::string &tekst, const int &naar) {
	bericht.setVan(telnr);
	bericht.setNaar(naar);
	bericht.setTekst(tekst);
}

void Mobiel::verzendNaam(const std::string & tekst, const std::string &contactNaam) {
	bericht.setVan(telnr);
...
LGCookie::LGCookie(Provider * provider, int tellnr) {
	this->provider = provider;
	this->telnr = tellnr;
	diagonalSize = 3;
	OS = "N/A";
...
const std::string LGCookie::mobielInfo() {
	std::cout << "Messages: " << "\n";
	for (int i = 0; i < berichten.size(); i++) {
		std::cout << "Telefoon LGCookie " << "OS: " << OS 
			<< " grootte: " << diagonalSize 
			<<" telnr: \"" + std::to_string(this->telnr) + "\"\nbericht: \"" 
			<< berichten.at(i).getTekst() << "\" van "
			<< std::to_string(berichten.at(i).getVan()) 
			<< " naar " << std::to_string(berichten.at(i).getNaar()) + "\n";
	}
	return "End of message list.";
}

Nokia3310::Nokia3310(Provider * provider, int tellnr) {
	this->provider = provider;
	this->telnr = tellnr;
	diagonalSize = 5;
	OS = "N/A";
...
const std::string Nokia3310::mobielInfo() {
	std::cout << "Messages: " << "\n";

	for (int i = 0; i < berichten.size(); i++) {
		std::cout << "Telefoon Nokia3310 " << "OS: " << OS << "grootte: " << diagonalSize << "telnr: \"" + std::to_string(this->telnr) + "\" bericht: \"" + berichten.at(i).getTekst() + "\" van " + std::to_string(berichten.at(i).getVan()) + " naar " + std::to_string(berichten.at(i).getNaar()) + "\n";
	}
